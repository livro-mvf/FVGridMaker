...
TEST(ErrorCodes, GridInputCodesAreStable) {
    EXPECT_EQ(
        error_code::kInvalidNVol,
        std::string_view{"FVGRID.GRID.INVALID_NVOL"}
    );
    EXPECT_EQ(
        error_code::kInvalidLength,
        std::string_view{"FVGRID.GRID.INVALID_LENGTH"}
    );
    EXPECT_EQ(
        error_code::kInvalidXInit,
...
TEST(ErrorCodes, CoordinateOrderCodesAreStable) {
    EXPECT_EQ(
...
TEST(ErrorCodes, YamlCodesAreStable) {
    EXPECT_EQ(
        ...
        std::string_view{"FVGRID.YAML.INVALID_NODE"}
    );
    EXPECT_EQ(
        error_code::kMissingYamlKey,
        std::string_view{"FVGRID.YAML.MISSING_KEY"}
    );
    EXPECT_EQ(
        error_code::kUnknownYamlPolicy,
        std::string_view{"FVGRID.YAML.UNKNOWN_POLICY"}
    );
}
