...
enum class CoreErr : std::uint16_t {
     ...
     * @details Ocorre quando um parâmetro não satisfaz as pré-condições
     *          (por exemplo: nulo, negativo, fora do domínio válido).
     */
    InvalidArgument = 1,

    /**
     * @brief Índice fora dos limites permitidos.
     * @details Geralmente associado a acessos a vetores ou malhas fora
     *          do intervalo [0, N-1].
     */
    OutOfRange = 2,
     ...
     * @details Usado em métodos em desenvolvimento ou pontos de extensão
     ...
     * @details Indica um estado lógico impossível, normalmente associado
     *          a bugs internos da biblioteca.
     */
    AssertFailed = 4,

    /**
     * @brief Geometria inconsistente detectada.
     * @details Exemplos: volumes negativos, faces com áreas nulas ou
     *          topologia inválida.
     */
 ...
 */
struct CoreErrorInfo {
    sv key;            ///< Chave textual única do erro (ex.: "CORE_INVALID_ARGUMENT").
    Severity severity; ///< Nível de severidade padrão associado ao erro.
    sv enUS;           ///< Mensagem padrão em Inglês, com placeholders (ex.: {name}, {index}).
    sv ptBR;           ///< Mensagem padrão em Português, com placeholders (ex.: {name}, {index}).
};

/**
 * @brief Mapeia um código `CoreErr` para seus metadados.
 *
 * Função `constexpr` que atua como uma tabela de consulta em tempo de
 * compilação, retornando as informações estáticas associadas a cada erro.
 *
 * @param e Código de erro do tipo `CoreErr`.
...
constexpr CoreErrorInfo get_core_error_info_data(CoreErr e) {
    switch (e) {
        ...
        case CoreErr::InconsistentGeometry:
            return {
                sv{"CORE_INCONSISTENT_GEOMETRY"},
                Severity::Error,
                sv{"Geometric inconsistency detected: {details}."},
                sv{"Inconsistência geométrica detectada: {details}."}
            };
        default:
 ...
 *
 * Verifica se o erro está dentro dos limites `_Min` e `_Max` antes de
 * efetuar a consulta, retornando uma estrutura vazia caso o código seja
 * inválido ou esteja fora do intervalo configurado.
 *
 * @param e Código de erro.
...
constexpr CoreErrorInfo get_core_info(CoreErr e) noexcept {
    if (static_cast<std::uint16_t>(e) >= static_cast<std::uint16_t>(CoreErr::_Min) &&
        static_cast<std::uint16_t>(e) <= static_cast<std::uint16_t>(CoreErr::_Max)) {
 ...
 *
 * Esta especialização permite que o sistema genérico de erros
 * (ErrorRegistry/Exceptions) obtenha, de forma uniforme, domínio,
 * severidade e mensagens associadas a cada valor de `CoreErr`.
 *
...
struct ErrorTraits<CoreErr> {
    ...
    static constexpr Severity default_severity(CoreErr e) noexcept {
        return detail::get_core_info(e).severity;
    }

    /**
     * @brief Obtém a chave textual única (string ID) do erro.
     */
    static constexpr std::string_view key(CoreErr e) noexcept {
        return detail::get_core_info(e).key;
    }
    ...
    static constexpr std::string_view enUS(CoreErr e) noexcept {
        return detail::get_core_info(e).enUS;
    }
    ...
    static constexpr std::string_view ptBR(CoreErr e) noexcept {
        return detail::get_core_info(e).ptBR;
    }
};
